...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'sqlite:///agrihelp.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
CORS(app, supports_credentials=True, resources={r"/*": {"origins": ["http://localhost:3000", "http://localhost:3002", "http://localhost:3003", "https://devulapellykushalhig.vercel.app"]}})
...
def handle_filters():
    ...
    if not crop:
        return jsonify({'error': 'Crop name is required'}), 400
    trends = trends_analyzer.get_crop_trends(crop, data.get('state'), data.get('year'))
    return jsonify({'trends': trends}), 200 if trends else (jsonify({'error': 'No data found'}), 404)
...
def predict_crop_health():
    ...
    try:
        ...
        crop, disease = (prediction.split('___') + ["Unknown"]*2)[:2]
        return jsonify({
